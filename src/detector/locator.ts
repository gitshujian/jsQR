...
export function countBlackWhiteRunTowardsPoint(origin: Point, end: Point, matrix: BitMatrix, length: number) {
  const switchPoints: Point[] = [origin];
...
function scorePattern(point: Point, ratios: number[], matrix: BitMatrix) {
  try {
    const horizontalRun = countBlackWhiteRun(point, {x: 0, y: point.y}, matrix, ratios.length);
    const verticalRun = countBlackWhiteRun(point, {x: point.x, y: 0}, matrix, ratios.length);

    const topLeftPoint = {
      x: Math.max(0, point.x - point.y),
      y: Math.max(0, point.y - point.x),
    };
    const topLeftBottomRightRun = countBlackWhiteRun(point, topLeftPoint, matrix, ratios.length);

    const bottomLeftPoint = {
      x: Math.min(matrix.width, point.x + point.y),
      y: Math.min(matrix.height, point.y + point.x),
    };
...
export function locate(matrix: BitMatrix) {
      ...
      if (v === lastBit) {
        ...
        if (validFinderPattern) {
          ...
          const matchingQuads = finderPatternQuads.filter(q =>
            q.bottom.y + 1 === y && Math.abs(q.bottom.startX - startX) + Math.abs(q.bottom.endX - endX) < 7,
          );
  ...
  const finderPatternGroups = finderPatternQuads
    ...
    .map(q => { // Initial scoring of finder pattern quads by looking at their ratios, not taking into account position
      ...
      const lengths = [q.top.endX - q.top.startX, q.bottom.endX - q.bottom.startX, q.bottom.y - q.top.y];
      const size = sum(lengths) / lengths.length;
      const score = scorePattern({x: Math.round(x), y: Math.round(y)}, [1, 1, 3, 1, 1], matrix) + variance(lengths);
    ...
    .map((point, i, finderPatterns) => {
      if (i > 4) { // Once we've tried 4 - give up
        return null;
      }
  ...
  const alignmentPatterns = alignmentPatternQuads
    .map(q => {
      ...
      const lengths = [q.top.endX - q.top.startX, q.bottom.endX - q.bottom.startX, q.bottom.y - q.top.y];
      const size = sum(lengths) / lengths.length;
      const sizeScore = scorePattern({x: Math.round(x), y: Math.round(y)}, [1, 1, 1], matrix) + variance(lengths);
